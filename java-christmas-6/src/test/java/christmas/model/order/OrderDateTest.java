...
class OrderDateTest {
    ...
    void setValidDate_exceptionTest(String given) {
        assertThatThrownBy(() -> new OrderDate(given)).isInstanceOf(IllegalArgumentException.class);
    }
    ...
    void setValidDate_normalTest(String input, int expected) {
        assertThatCode(() -> new OrderDate(input)).doesNotThrowAnyException();
        assertEquals(new OrderDate(input).getDate(), expected);
    }
}